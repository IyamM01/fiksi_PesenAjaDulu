...
/// Service for fetching orders from the backend API
class OrderApiService {
  ...
  /// Fetch all orders for the authenticated user from /order endpoint
  Future<List<OrderListModel>> fetchUserOrders() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData is Map<String, dynamic>) {
          ...
          if (responseData.containsKey('data')) {
            ...
            return dataList
                .map(
                  (orderJson) => OrderListModel.fromJson(
          ...
          return responseData
              .map(
                (orderJson) =>
                    OrderListModel.fromJson(orderJson as Map<String, dynamic>),
              )
              .toList();
        } else {
          throw Exception('Unexpected response format from /order endpoint');
        }
      } else {
      ...
      throw Exception('Failed to fetch orders from API: $e');
    }
  }
